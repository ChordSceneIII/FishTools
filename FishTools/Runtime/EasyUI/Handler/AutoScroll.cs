    ...
    public class AutoScroll : MonoBehaviour
    {
        ...
        void AutoFixedScrollBar()
        {
            ...
            if (selected != null && selected.transform.IsChildOf(ScrollRect.content))
            {
                ...
                //获取相对位置
                var direction = FishUtility.CompareRectDirection(selRec, scrollRec, true);

                //不在对象内部时，对滚动条value进行调整直到完全被显示
                if (direction != new Vector2(0, 0))
                {
                    float DeltaX = selRec.rect.width / ScrollRect.content.rect.width / 4;
                    float DeltaY = selRec.rect.height / ScrollRect.content.rect.height / 4;

                    ScrollRect.horizontalNormalizedPosition += rateX * direction.x * DeltaX;
                    ScrollRect.verticalNormalizedPosition += rateY * direction.y * DeltaY;
                }
            }
        }
    }
